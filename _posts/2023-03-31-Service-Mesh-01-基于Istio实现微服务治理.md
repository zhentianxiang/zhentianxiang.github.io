...
istio是一个服务网格所谓服务网格就是在服务的网络层面形成了一种抽象的网络，服务与服务之间调用主要被istio进行接管，istio会代替k8s中的kube-proxy服务，使其集群中的服务网络流向都走向istio。istio的实现原理大致如下：首先要想自己的服务被istio进行接管首先要以sidecar方式往容器中注入一个istio- proxy容器，该容器里面主要有两个服务一个是piolot-agent一个是envoy，piolot-agent属于istio控制端的一部分负责管理和配置整个服务网格中的代理，它主要扮演以下几个重要角色：1.服务发现和负载均衡2.配置下发3.故障检测和修复4.TLS加密和证书管理。而envoy主要是负责流量代理，它的工作方式类似于nginx的代理，其原理大致一致，这也就是为什么istio为什么会实现微服务的流量治理。当然首先能够实现在k8s中将kube-proxy的流量给拦截过来主要是因为在启动istio-proxy容器之前的init初始化容器，该容器完成了一些iptables规则的配置，主要是负责把所有的入站流量给拦截到istio-proxy容器再由envoy容器进行代理转发，所以说在k8s中如果pod被istio管理，即使你把kube-proxy给停掉你也能正常访问pod中的服务
