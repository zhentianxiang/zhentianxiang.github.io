...
RUN  mkdir /etc/yum.repos.d/bak && mv /etc/yum.repos.d/*.repo /etc/yum.repos.d/bak && \
     ...
     yum makecache && \
     yum -y install sudo gcc make wget curl git telnet libffi-devel openssl-devel bzip2-devel expat-devel gdbm-devel readline-devel sqlite-devel && \
...
```sh
# 根据情况可以选择性使用代理
$ docker build . -t chatgpt-wechat:py-v1 、
...
$ docker run -dit --name chatgpt-wechat -v /data/chatgpt/v1/config.json:/chatgpt-on-wechat/config.json chatgpt-wechat:py-v1
```
