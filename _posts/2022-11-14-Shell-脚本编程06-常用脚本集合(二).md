...
#HarborAPI              2.0
#*****************************************************************************
Harbor_User=zhentx                                       # Harbor的用户
Harbor_Passwd=xxxx                                # Harbor的密码
Harbor_Address=https://hub.adtecmall.com                 # Harbor地址
Registry_Address=${Harbor_Address#https://}              # docker pull 拉取
# 授权登录
docker login $Harbor_Address -u $Harbor_User -p $Harbor_Passwd
if [ $? == 0 ] ;then
        echo -e "\033[32m仓库login成功!\033[0m"
else
        echo -e "\033[31m仓库login失败,请检查授权!\033[0m"
        exit 0
fi
# 获取所有项目列表
Project_List=$(curl -s -k -u "$Harbor_User:$Harbor_Passwd" -X GET "$Harbor_Address/api/v2.0/projects?page=1&page_size=100&with_tag=true&with_label=false&with_scan_overview=false&with_signature=false&with_immutable_status=false" | jq -r '.[].name')
# 自定义数组
#Project_List=(
#"nginx"
#"nginx2"
...
for Project in $Project_List; do
    # 循环获取远端项目下所有的镜像
    GET_Names=$(curl -s -k -u "$Harbor_User:$Harbor_Passwd" -X GET "$Harbor_Address/api/v2.0/projects/$Project/repositories?page=1&page_size=100&with_tag=true&with_label=false&with_scan_overview=false&with_signature=false&with_immutable_status=false" | jq -r '.[].name')
    for Full_Image_Name in $GET_Names; do
        # 去掉项目名称，只保留镜像名称
        ...
        if [ $Slash_Count -ge 1 ]; then
            # 多层镜像名称，需要进行编码处理
            Encoded_Part=$(echo "$Image_Name" | sed 's/\//%252F/g')
            # 获取多层镜像的所有版本(tag)
            Get_Tags=$(curl -s -k -u "$Harbor_User:$Harbor_Passwd" -X GET "$Harbor_Address/api/v2.0/projects/$Project/repositories/$Encoded_Part/artifacts?page=1&page_size=100&with_tag=true&with_label=false&with_scan_overview=false&with_signature=false&with_immutable_status=false" | jq -r 'map(select(.tags != null)) | .[].tags[].name')
        else
            # 单层镜像名称，直接处理
            Get_Tags=$(curl -s -k -u "$Harbor_User:$Harbor_Passwd" -X GET "$Harbor_Address/api/v2.0/projects/$Project/repositories/$Image_Name/artifacts?page=1&page_size=100&with_tag=true&with_label=false&with_scan_overview=false&with_signature=false&with_immutable_status=false" | jq -r 'map(select(.tags != null)) | .[].tags[].name')
        fi

        # 格式化输出远端镜像信息，并存储到数组中
        if [ -n "$Get_Tags" ]; then
            for tag in $Get_Tags; do
                echo -e "\033[36m查找到 $Registry_Address/$Full_Image_Name:$tag\033[0m"
            done
        fi
    done
done
...
#*****************************************************************************
Harbor_User=zhentx                                       # Harbor的用户
Harbor_Passwd=xxxxxx                                # Harbor的密码
Harbor_Address=https://hub.adtecmall.com                 # Harbor地址
Registry_Address=${Harbor_Address#https://}              # docker pull 拉取
# 授权登录
docker login $Harbor_Address -u $Harbor_User -p $Harbor_Passwd
...
for Project in $Project_List; do
    ...
    for Full_Image_Name in $GET_Names; do
        ...
        if [ $Slash_Count -ge 1 ]; then
            ...
            Encoded_Part=$(echo "$Image_Name" | sed 's/\//%252F/g')
            # 获取多层镜像的所有版本(tag)
            Get_Tags=$(curl -s -k -u "$Harbor_User:$Harbor_Passwd" -X GET "$Harbor_Address/api/v2.0/projects/$Project/repositories/$Encoded_Part/artifacts?page=1&page_size=100&with_tag=true&with_label=false&with_scan_overview=false&with_signature=false&with_immutable_status=false" | jq -r 'map(select(.tags != null)) | max_by(.push_time) | .tags[0].name')
        else
            # 单层镜像名称，直接处理
            Get_Tags=$(curl -s -k -u "$Harbor_User:$Harbor_Passwd" -X GET "$Harbor_Address/api/v2.0/projects/$Project/repositories/$Image_Name/artifacts?page=1&page_size=100&with_tag=true&with_label=false&with_scan_overview=false&with_signature=false&with_immutable_status=false" | jq -r 'map(select(.tags != null)) | max_by(.push_time) | .tags[0].name')
        fi
...
还可以配置 save 功能进行使用，使其可以自动拉取镜像并且save到本地，但是我这个脚本是拉取的镜像是特定定义的两个测试项目，没有获取全部项目的镜像，而且是拉取的最新的镜像
```sh
#!/bin/bash
...
#for Project in $Project_List; do
    ...
    for Full_Image_Name in $GET_Names; do
        ...
        if [ -n "$Get_Tags" ]; then
            for tag in $Get_Tags; do
                echo -e "\033[36m查找到 $Registry_Address/$Full_Image_Name:$tag\033[0m"
                Image_Tags+=("$Registry_Address/$Full_Image_Name:$tag")
            done
        fi
    done
done
